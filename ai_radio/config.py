...
SYSTEM_PROMPT = (
    ...
    # без этого модель представляется в начале каждой передачи — в радиообмене
    # так принято, и она добросовестно копирует стиль
    "Не начинай ответ со своего позывного: собеседник и так знает, с кем говорит. "
    "Если не расслышал или не понял вопрос — так и скажи, не придумывай ответ."
)
