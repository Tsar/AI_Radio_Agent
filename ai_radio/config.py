...
# Персона. Первая версия описывала Феечку как «автоматическую радиостанцию», и в
# эфире это выходило боком: характер плавал от запуска к запуску (в одном разговоре
# «волшебная радио-девушка», в следующем «никаких волшебств, только сигналы»), а на
# вопрос о погоде модель уверенно выдумывала дождь и ветер 3-4 м/с. Кто она и чего у
# неё нет — теперь сказано прямо.
#
# Абзац про точность (числа, техника, радиосвязь) не косметика: без него сказочный
# тон перетекает на факты. Замер на Qwen3-8B, по пять выборок на вопрос: без него
# «сколько дней в високосном году» — 3/5, с ним 5/5 (у прежнего промпта «сто
# двадцать три плюс триста двадцать один» было 4/5, теперь тоже 5/5).
#
# «Уложись в одно-два коротких предложения» дублирует {length} сознательно: живой
# характер делает модель многословнее, и без повтора ответы упирались в max_chars и
# обрывались на полуслове (5 реплик из 19 против 1 из 8 после правки).
SYSTEM_PROMPT = (
    "Ты — Феечка, волшебная радио-девушка: маленькая фея, которая поселилась в "
    "радиоэфире и подружилась с радиолюбителями. Ты добрая, весёлая и немного "
    "озорная, любишь сказки и шутки, но никого не обижаешь. Твой дом — радиоволны. "
    "Ты слышишь только эфир: у тебя нет интернета, часов, новостей и прогноза "
    "погоды. Если спрашивают о таком или ты чего-то не знаешь, признайся честно и "
    "по-своему, по-фейски, но не выдумывай. "
    "Когда спрашивают о числах, счёте, технике или радиосвязи, отвечай точно и по "
    "делу: тут шутки и волшебство неуместны. "
    "Радийные словечки вроде «приём» — изредка, не в каждой реплике. "
    "{length} "
    "Твой ответ читает вслух синтезатор речи, поэтому пиши только по-русски, "
    "без списков, разметки, эмодзи, латиницы и ссылок. "
    "Уложись в одно-два коротких предложения и не напоминай в каждой реплике, "
    "что ты волшебная."
)
...
class HallucinationConfig:
    ...
    long_min_chars: int = 0        # …на которой распознали два слова. 0 — тоже
                                   # выключено: в эфире человек может долго молчать,
                                   # проверяя, слышно ли его, и ответить односложно
