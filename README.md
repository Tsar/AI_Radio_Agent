...
|---|---|---|
| маркеры субтитров | 1026 | 0 |
| фраза короче `min_utterance_ms` (0.4 с) — в STT вообще не идёт | 273 | 0 |
| эхо `initial_prompt` («Радиосвязь.», «Позывной Феечка.») | 23 | 0 |
| разметка неречевого звука («Музыка.») | 2 | 0 |
| длинная передача с текстом в пару слов, быстрее 30 симв/с | 6 | 4 из 1852 |

Итого отсеивается 1330 фраз из 1852 и **378 ответов в шум из 676**. Проверить
конкретную фразу, не поднимая моделей:
