...
**Клиент-сервер — готов:** тот же конвейер, разрезанный по шву `Responder`. У рации
работает тонкий клиент (звук, VAD, PTT — только numpy, sounddevice, pyserial), а
Whisper, LLM, Piper и RVC считает `main.py serve` на машине с видеокартой, обычно
в той же локальной сети. Включается `--responder remote --server http://мозг:8082`.
...
Машина с видеокартой не обязана стоять рядом с рацией. Конвейер режется по тому же
шву `Responder`: репитер, VAD, звуковая карта и PTT остаются у рации, а `respond()`
уезжает по HTTP на сервер, где живёт весь тяжёлый стек.

```
      у рации (клиент)                                машина с видеокартой (сервер)
 ┌───────────────────────────────┐   POST /respond    ┌───────────────────────────────┐
 │ MicSource → EnergyVAD →       │ ── WAV фразы ────▶ │ main.py serve                 │
 │ Repeater ──▶ RemoteResponder  │                    │  Whisper → позывной → LLM →   │
 │          ◀── PTT + Speaker    │ ◀── WAV ответа ─── │  Piper → RVC                  │
 │ numpy, sounddevice, pyserial  │   или 204 = молчим │  + llama-server, RVC-сервис   │
 └───────────────────────────────┘                    └───────────────────────────────┘
```

Протокол повторяет RVC-сервис — WAV в обе стороны, так что его можно потрогать
curl'ом: `curl --data-binary @фраза.wav -o ответ.wav http://мозг:8082/respond`.
Вместе с ответом сервер отдаёт в заголовке строки своего журнала по этой фразе
(`[STT] …`, `[--] не нам — молчим`, `[LLM] …`), и клиент печатает их у себя: у рации
видно, что услышал Whisper и почему агент молчит, без хождения на сервер. Строка
`[NET] сервер: 0.9 с (из них сеть 0.02 с)` показывает, сколько добавила сеть.

Строгий half-duplex сохраняется: вход стоит на паузе на всё время «спросили сервер +
передали». Обрывки короче `min_utterance_ms` клиент отбрасывает сам, не гоняя их по
сети. Один сервер — один разговор: позывной, окно диалога и история живут на нём.
Клиенту всё равно, кто на том конце: `serve --responder parrot` даёт эхо, чтобы
проверить связь и PTT до того, как поставлены модели.

Отказ сервера или сети — не падение: клиент пропускает фразу, пишет `[ERR]` и
продолжает слушать эфир; вернувшийся сервер подхватывается на следующей фразе.
На старте клиент сообщает, какой мозг нашёл по адресу: позывной, модель STT, RVC.
...
автозапуск, проверка офлайн-готовности и диагностика. Тонкий клиент у рации на
отдельной машине ставится по [CLIENT.md](CLIENT.md) — пошагово, без остального стека.
...
- Клиенту у рации (`--responder remote`) достаточно `requirements.txt`: модели,
  faster-whisper и piper ставятся только на сервере (`main.py serve`)
...
тестах и в проде разойдётся. Видеопамять считайте **по пикам, а не по покою**: сумма в
покое вводит в заблуждение, потому что и Whisper на транскрипции, и RVC на длинной
фразе берут заметно больше. Замеры с обеих прод-машин — в [DEPLOY.md](DEPLOY.md),
раздел 8: стек с Qwen3-4B занимает 5029 из 5120 МиБ на пятигигабайтной карте и
8938 из 10240 с Qwen3-8B на десятигигабайтной.
...
Whisper обучен на субтитрах и на шуме договаривает то, чем субтитры кончаются:
«Продолжение следует», «Редактор субтитров А.Семкин», «Субтитры создавал DimaTorzok».
В первые сутки работы на даче таких фраз оказалось **1224 из 1852**, и агент ответил
на 330 из них — окно диалога после позывного открыто, а галлюцинация приходит как
обычная реплика, продлевая его дальше. Разговор превращался в бормотание с шумом.

**Уверенность модели тут не помогает.** Замер на реальном шуме с прода:

| | значение | наш порог | сработал? |
|---|---|---|---|
| `no_speech_prob` | **0.000** | > 0.6 | нет |
| `avg_logprob` | **−0.276** | < −1.0 | нет |
| `compression_ratio` | 0.83 | > 2.4 | нет |

Whisper уверен, что слышит речь. Поднимать пороги нельзя — под нож пойдёт живая
речь с рации, которая и так на пределе разборчивости. Поэтому фильтр текстовый
(`ai_radio/hallucinations.py`), и каждое правило проверено на том же журнале:

| Правило | Ловит | Ложных |
|---|---|---|
| титр как целое предложение | 1100 | 0 |
| фраза короче `min_utterance_ms` (0.4 с) — в STT вообще не идёт | 276 | 0 |
| эхо `initial_prompt` («Радиосвязь.», «Позывной Феечка.») | 24 | 0 |
| разметка неречевого звука («Музыка.») | 2 | 0 |
| быстрее 30 симв/с («Ту-ту-ту-ту…», «Уууу…») | 1 | 0 |

Итого отсеивается 1404 фразы из 1978 и **364 ответа в шум из 695**.

**Титр ищется как отдельное предложение, а не как слово.** Первая версия искала
подстроку — и забраковала живое «Феечка, расскажи мне про субтитры» вместе с «Надо
было узнать, что такое корректор Егорова». Хуже того, Whisper дописывает титр в
конец настоящей фразы:

    Феечка услышала, что ты в рацию говоришь… отнеси уши в комнату. Субтитры сделал DimaTorzok

Здесь реплика живая, галлюцинация — только хвост, и выбрасывать надо его, а не всё
целиком: иначе человек в эфире не понимает, почему его не слышат. Поэтому титры
сначала **срезаются** (`strip_subtitles`), и лишь остаток проходит правила. В журнале
это видно как `(срезан титр: «…»)`. На реальных данных так спаслись 8 фраз с хвостом
и ещё два десятка живых реплик про субтитры.

Граница предложения — точка **с пробелом**: иначе инициалы в «Редактор субтитров
А.Семкин Корректор А.Егорова» разорвали бы титр на три куска, и ни один не совпал бы
с шаблоном.

Проверить конкретную фразу, не поднимая моделей:

```bash
.venv/bin/python main.py hallucination-test "Продолжение следует." --duration 4.4
```

**Два правила по умолчанию выключены**, оба — по одной причине: они судят о речи по
её плотности, а в эфире плотность бывает любой.

- `min_chars_per_s` («слишком медленно») резал живые «Да», «Угу», «Федька, привет».
- `long_min_chars` («длинная передача, а текста на два слова») резал «Хорошо.» на
  14 с и «Так, ну вот, хорошо.» на 21 с. На радиосвязи это штатный случай: человек
  вышел на связь, разбирается, слышно ли его, и только потом коротко отвечает. Все
  титры, которые правило ловило, и так снимаются шаблонами выше — включение стоило
  бы 4 живых фраз из 1978 и не дало бы ни одной новой галлюцинации.

Включаются установкой ненулевого порога в `HallucinationConfig`.
...
# петличный передатчик DJI:                            threshold 0.00132
# встроенный микрофон десктопа:                        threshold 0.01696
```
...
Живой конвейер можно проверить без радио: микрофон играет роль «приёма», динамик —
«передачи», PTT — заглушка. Колонки годятся: вход стоит на паузе всё время ответа и
передачи, поэтому сам себя агент не слышит. Наушники надёжнее — при низком пороге
можно зацепиться за эхо комнаты уже после того, как вход снова открылся.
...
### Клиент у рации, мозг на сервере

На машине с видеокартой — те же llama-server и RVC-сервис, что и для `--responder llm`,
плюс сервер мозга. Флаги моделей, позывного и RVC — здесь, у него:

```bash
.venv/bin/python main.py serve --profile vram10 --rvc      # слушает 0.0.0.0:8082
curl -s http://127.0.0.1:8082/health                       # {"status": "ok", "callsign": "феечка", …}
```

У рации — клиент, которому нужны только `requirements.txt` и звук:

```bash
.venv/bin/python main.py run --live --responder remote --server http://192.168.1.10:8082 \
    --threshold 0.0012 --ptt txdbreak --port /dev/ttyUSB0
```

Пошаговая установка клиента на чистой машине — [CLIENT.md](CLIENT.md).
Всё, что про железо — порог, `--device-rate`, устройства, PTT, — остаётся флагами
клиента. Флаги мозга на клиенте не действуют, он об этом предупредит. Если у машины
с мозгом есть публичный интерфейс, слушать надо только LAN: `serve --host 192.168.x.x`. Проверить связь
до установки моделей: `main.py serve --responder parrot` на сервере — клиент получит
своё эхо. Замерить конвейер вместе с сетью: `main.py bench --in-file запись.mp3
--server http://192.168.1.10:8082`. Авторизации у сервера нет, он для локальной сети —
...
| `llm.max_sentences` | 2 | основной ограничитель длины; бессилен на тексте без точек |
| `llm.max_chars` | 110 | рубеж под ним: режет по границе предложения, иначе по слову; задаётся пресетом |
| `tts.max_seconds` | 14.0 | потолок эфира по готовому аудио; 0 — без ограничения |
...
| `remote.base_url` | `http://127.0.0.1:8082` | сервер мозга для `--responder remote` (`--server`) |
| `remote.timeout_s` | 60 | ожидание ответа сервера целиком: STT + LLM + TTS + RVC |
...
Одним флагом `--reply-length`, потому что крутить ручки порознь бессмысленно:
...
полуслове). Пресет меняет их согласованно.

| Пресет | Предложений | Символов | Потолок эфира | |
|---|---|---|---|---|
| `short` | ≤ 2 | 110 | 14 с | по умолчанию |
| `medium` | ≤ 4 | 260 | 30 с | полноценный ответ, канал ещё не блокирует |
| `long` | ≤ 8 | 520 | 55 с | **осторожно**, см. ниже |

Последние две колонки — нижние рубежи, и появились они не из аккуратности.
**`max_sentences` беззащитен перед текстом без знаков препинания.** На «прочитай
Лукоморье» и «посчитай до тридцати» Qwen3 выдаёт сплошной поток без единой точки,
`split` по `[.!?]` возвращает одну строку целиком, резать нечего — и в эфир уходит
17 с вместо шести. В проде это ещё и роняло стек: RVC на такой фразе выбирал всю
свободную видеопамять (DEPLOY.md, раздел 8). Поэтому:

- **`max_chars`** режет нормализованный текст, по возможности на границе предложения:
  лучше передать фразу короче, но целую. Если законченной мысли в пределах лимита нет
  (сплошной поток без точек), режет по слову и снимает висящий союз или предлог —
  иначе передача кончается на «…из травы и соломы, и.», и это слышно в эфире;
- **`max_air_s`** (`tts.max_seconds`) — последний рубеж уже по готовому аудио, с
  затуханием в конце; срабатывает **до** отправки в RVC и пишет в журнал
  `[TTS] ответ обрезан`.

На той же провокации: было 17.67 с эфира, стало 11.93 с. Нормальные ответы (35–75
символов) ни один из рубежей не трогает.
...
ai_radio/
  ...
  bench.py       # замер задержки по звеньям на реальной записи (и через сервер)
  remote.py      # RemoteResponder: фраза на сервер по HTTP (WAV), ответ обратно — stdlib
  server.py      # main.py serve: LLMResponder за http.server, журнал фразы — клиенту
  engines/
...
main.py          # CLI: calibrate | run [--live] | serve | bench | trigger-test | devices
tools/
...
- Тракт с рацией: аттенюатор, развязка и проверка, что эфир слышен на второй
  станции. Единственное, что осталось непроверенным вживую, — PTT-линия и логика
  уже проверены.
- Логирование всех сообщений, включая аудио, и веб-интерфейс для просмотра истории.
  Естественный шов — `LLMResponder.respond()`: там уже есть и текст STT, и ответ
  LLM, и тайминги, а на входе и выходе аудио в едином формате.
