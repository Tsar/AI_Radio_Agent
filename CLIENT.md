...
| Один раз `[MIC] поток не возобновился … открываю заново` | норма для устройств, выбранных через PulseAudio/PipeWire (в имени `alsa_input.…`): этот бэкенд не умеет возобновлять поток после паузы, клиент открывает его заново сам |
