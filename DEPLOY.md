...
- **На карте живёт сосед** (чужой сервис, ~1 ГБ), которому надо оставлять
  **не меньше 1.5 ГБ**. Замер на самой машине, `-c 4096`, `large-v3`, RVC fp32:
  ...
  |---|---|
  | сосед | 982 МиБ |
  | llama-server, `-c 4096` | 3172 МиБ |
  | RVC fp32, `pm`, после прогрева | 782 → 802 МиБ |
  | `main.py serve`, `large-v3` int8 | 1662 → 1674 МиБ |
  | **пик всей карты во время bench** | **7256 из 10240 МиБ** |

  То есть свободно 2.9 ГБ при требуемых соседу 1.5 — запас есть, ужимать
  контекст LLM, как на даче, не нужно.
- Замер `bench --server` на самом сервере: **2.02 с** на фразу (STT 1.05 / LLM 0.39 /
  TTS+RVC 0.44), сеть 0.02 с. Бюджет 10 с.
