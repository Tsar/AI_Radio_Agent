...
**Целевая машина:** Dell OptiPlex 790 — i7-2600 (без AVX2), 8 ГБ RAM, Ubuntu 22.04
(системный Python 3.10), Sound Blaster Live! 5.1, видеокарта — см. раздел «Железо».

**Ключевое отличие от dev:** интернет есть **только во время настройки**. На даче его
не будет, поэтому всё, что скачивается «на лету», должно быть скачано и проверено
заранее — см. «Проверка офлайн-готовности».
...
| голос Piper | 61 МБ |
| Whisper `large-v3` в HF-кэше | 2.9 ГБ |
| **RVC — итого** | **8.9 ГБ** |
| код + `.git` | 53 МБ |
...
только для `torch.compile`) → Whisper `medium` вместо `large-v3` (−1.4 ГБ, ценой
качества) → перенести веса и venv'ы на HDD, их в OptiPlex три штуки.

---

## 2. Проверка офлайн-готовности — делать обязательно
...
du -sh ~/.cache/huggingface/hub/models--Systran--faster-whisper-large-v3
...
- [ ] Видеокарта: `nvidia-smi` видит, `compute_cap` ожидаемый, драйвер ветки ≤580
...
- [ ] Питание: хватает ли БП под выбранную карту (для P102-100 — 250 Вт, нужен другой БП)
...
2. `main.py bench --in-file запись.mp3 --profile prod --threshold ПОРОГ` — реальные
   времена на этом железе. Ожидание: ~1.2–1.8 с на фразу, основной вклад даст Piper
   на слабом CPU. Бюджет 10 с.
