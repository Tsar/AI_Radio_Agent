...
# --max-segment 6: длиннее — режем на куски, иначе пик растёт с длиной фразы и
...
ExecStart=%h/Retrieval-based-Voice-Conversion-WebUI/.venv/bin/python \
          infer-http-service.py --port 8081 --fp32 --max-segment 6
...
| RVC отдаёт 500 на длинной фразе, короткие идут нормально | пик RVC растёт с длиной, если вход не режется. Проверить `curl -s 127.0.0.1:8081/health` — в `chunking` должно быть `x_pad: 1` и `x_max: 9` (то есть `--fp32 --max-segment 6` доехали). Раздел 8 |
...
Запас около 580 МиБ — на коротких фразах. Длинная его съедает, см. ниже.
