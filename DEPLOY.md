...
| Диск | SSD 224 ГБ: `/` на 120 ГБ, свободно было 46 ГБ | SSD: `/` на 123 ГБ, свободно 42 после установки; **HDD `/mnt/2TB`**, 860 ГБ свободно — там модели |
| Звук | Sound Blaster Live! 5.1 | — |
| Профиль STT | **`--profile vram5`** (turbo) | **`--profile vram10`** (large-v3) |
| LLM | Qwen3-4B `Q4_K_M` | **Qwen3-8B `Q4_K_M`** (см. раздел 6) |
...
- **Драйвер 535 против CUDA 12.4**, с которой собран llama.cpp. По правилу
  minor-version compatibility этого достаточно (нужен ≥525) — проверено 12.09.2026:
  `llama-server --list-devices` видит `CUDA0: NVIDIA P102-100 (10150 MiB)`, `/health`
  отвечает, модель считает на карте.
- **Диски: код и оба venv на SSD (`/`), тяжёлые модели на HDD `/mnt/2TB`.** Раздел
  принадлежит root, каталог под нас выдаётся один раз: `sudo mkdir -p /mnt/2TB/ai-radio
  && sudo chown irv:irv /mnt/2TB/ai-radio`. GGUF и кэш Whisper лежат там (симлинк
  `models/llm` и `HF_HOME` в юните мозга); venv на HDD не переносить — тысячи мелких
  файлов, импорт torch упирается в seek'и. Подробности и замеры — в конце раздела 6.
...
- [x] PTT: линия TX USB-UART. Проверено 12.08.2026 **по светодиоду на самом
      адаптере** (CH341), не осциллографом: агент жмёт передачу на свои ответы,
      диод горит ровно между `[PTT] KEY` и `[PTT] UNKEY`. Инверсия по умолчанию
      (`invert=True`) верна — диод загорается при «break снят, TX high», как и
      пишет журнал. Осциллограф остаётся нужен для проверки уровней на входе
      рации, когда будет собран тракт
...
# --max-segment 4: длиннее — режем на куски, иначе пик растёт с длиной фразы и
# длинный ответ уводит в OOM весь стек (раздел 8). На 10 ГБ не нужно.
ExecStart=%h/Retrieval-based-Voice-Conversion-WebUI/.venv/bin/python \
          infer-http-service.py --port 8081 --fp32 --max-segment 4
...
нужен только там, где видеопамять впритык: на домашней машине экран входа gdm берёт
4 МиБ, и стек помещается вместе с ним. По умолчанию сервер слушает `0.0.0.0:8082`
**без авторизации** — он для локальной сети. Если у машины есть публичный интерфейс,
...
**Что на домашней машине иначе** (развёрнуто 12.09.2026, по этому чеклисту):
...
- **У машины два интерфейса, на одном публичный адрес.** Сервер привязан к
  ...
  по публичному адресу порт 8082 закрыт. Файрвол там — **iptables руками, не ufw**
  (юнит `ufw` активен, но правила свои): вход с LAN-интерфейса открыт целиком,
  разрешать порт не пришлось.
- RVC — без `--fp32` (P102-100 ловится авто-детектом, `/health` показывает
  `"is_half": false`), но **с `--max-segment 4`**, как на даче: без нарезки пик RVC
  растёт с длиной фразы, и на 14 с (потолок short) он берёт 2.2 ГБ вместо 0.8 —
  с Qwen3-8B соседу на карте не оставалось его 1.5 ГБ. С нарезкой пик на той же
  фразе 1.4 ГБ. Прогрев: 4.0 / 0.5 / 1.2 с.
...
- **На карте живёт сосед** (чужой сервис, ~1 ГБ), которому надо оставлять
  **не меньше 1.5 ГБ**. Замер на самой машине с Qwen3-8B, `-c 2048` + KV `q8_0`,
  `large-v3`, RVC fp32 с `--max-segment 4`:

  | Процесс | VRAM |
  |---|---|
  | сосед | 982 МиБ, до 1.5 ГБ |
  | llama-server, Qwen3-8B, `-c 2048`, KV `q8_0` | 4850 МиБ |
  | RVC fp32, `pm`, `--max-segment 4` | 762 → 1430 МиБ на фразе 14 с |
  | `main.py serve`, `large-v3` int8 | 1662 → 1674 МиБ |
  | **пик всей карты (RVC на 14 с)** | **8938 из 10240 МиБ** |

  Запас 1.3 ГБ, из них до 0.5 может забрать сосед — остаётся ~0.8 ГБ. Если станет
  тесно, `--profile vram5` (turbo вместо large-v3) вернёт ещё ~0.8 ГБ: на записи с
  рации turbo распознавал не хуже.
- **LLM здесь Qwen3-8B, а не 4B.** Сравнение кандидатов 12.09.2026 на dev (наш
  промпт, `short`, температура 0.7, десять радийных вопросов, по три выборки на
  арифметику и високосный год):

  | Модель | 15 + 27 | Високосный год | Речь | VRAM, `-c 2048` |
  |---|---|---|---|---|
  | Qwen3-4B `Q4_K_M` | ошибалась | верно | норм | 2.7 ГБ |
  | **Qwen3-8B `Q4_K_M`** | верно ×4 | 366 ×3 | ровная, без артефактов | 4.8 ГБ |
  | Qwen3-14B `Q4_K_M` | верно | один раз 365 | норм | 9.5 ГБ, не влезает |
  | Qwen3-4B-Instruct-2507 | верно | верно | болтливее | 2.7 ГБ |
  | Qwen3.5-4B `Q6_K` | «сорока четыре» ×2 | «три шести дней» | артефакты | 3.8 ГБ |
  | Qwen3.5-9B `Q4_K_M` | верно | **365 ×3** | болтливая, коверкает числа | 5.3 ГБ |
  | Gemma-3-12B `Q4_K_M` | верно | **365 ×3** | выдумала сказку вместо Лукоморья | 6.4 ГБ |
  | Qwen3-30B-A3B-2507, эксперты в RAM | один раз «семьдесят два» | 366 ×3 | ровная | 1.3 ГБ + 18.6 ГБ RAM, 19 ток/с |

  Qwen3.5 (март 2026) в режиме без размышлений по-русски хуже старой Qwen3-8B, а
  режим с размышлениями в бюджет эфира не влезает. Перенос Whisper и RVC на CPU
  ради модели покрупнее отвергнут замером на самом Xeon: Whisper turbo 5.3 с на
  фразу 11 с (на GPU 0.9), RVC 2.4 с на короткий ответ (на GPU 0.35). MoE
  30B-A3B с экспертами в RAM — запасной вариант «умнее, но 1.7 с на ответ и
  18.6 ГБ RAM»; на десяти вопросах преимущества перед 8B не показала.
  Вызывную частоту двухметрового диапазона не знает ни одна из них.
...
| Агент отвечает сам себе, в журнале «Продолжение следует» и «Редактор субтитров» | галлюцинации Whisper на шуме. Фильтр в `ai_radio/hallucinations.py` их снимает; проверить конкретную фразу — `main.py hallucination-test "текст" --duration 4.4`. Если фраза новая — добавить маркер в `SUBTITLE_MARKERS` (README, «Галлюцинации STT») |
| Живая короткая реплика («Да», «Угу») пропала из журнала | включён `min_chars_per_s` — по умолчанию он 0 именно поэтому. Либо фраза короче `min_utterance_ms` (0.4 с) |
...
| RVC отдаёт 500 на длинной фразе, короткие идут нормально | пик RVC растёт с длиной, если вход не режется. Проверить `curl -s 127.0.0.1:8081/health` — в `chunking` должно быть `x_pad: 1` и `x_max: 7` (то есть `--fp32 --max-segment 4` доехали). Раздел 8 |
| Агент перезапускается по кругу, в журнале один `CUDA failed with error out of memory` за другим | памяти не хватает **соседу**, а рестарт агента её не возвращает: после своего OOM RVC держит весь пул (1.16 ГиБ вместо 706 МиБ). `systemctl --user restart ai-radio-rvc` — перезагрузка машины не нужна. Начиная с 11.08.2026 агент такую фразу пропускает и остаётся в эфире, так что цикл рестартов означает старую версию кода |
| В эфир ушла передача на 15+ секунд | ответ не подрезан: `max_sentences` бессилен, когда модель не ставит точек (стихи, счёт по порядку). Смотреть `[TTS] ответ обрезан` в журнале; пределы — `max_chars` и `max_air_s` в `REPLY_LENGTHS` |
...
| Клиент пишет `[warn] сервер не отвечает` или `[ERR] … сервер недоступен` | на машине с видеокартой не поднят `ai-radio-server`, либо в `SERVER` не тот адрес. С клиентской машины: `curl -s http://адрес:8082/health`. Проверить, какой адрес в `--host` (журнал мозга: «Сервер слушает …»), дальше — файрвол между ними |
...
Снято на самой дачной машине (Quadro P2000, 5120 МиБ), стек поднят целиком:

| Модель | Роль | Где считает | VRAM: покой → пик | Веса на диске |
|---|---|---|---|---|
| **Qwen3-4B** `Q4_K_M`, llama.cpp | ответы | **GPU** (`-ngl 99`) | 2682 МиБ (с KV `q8_0`, ctx 2048) | 2.4 ГБ |
| **Whisper `large-v3-turbo`**, CTranslate2 `int8` | распознавание | **GPU** | 1124 → **1380** на транскрипции | 1.6 ГБ |
| **RVC** `voicevox_speaker_43` + HuBERT, PyTorch **fp32** | тембр | **GPU** | 730 → **1206** на длинной фразе | 53 МБ + 181 МБ |
| **Piper** `ru_RU-irina-medium`, onnxruntime | синтез речи | **CPU** | — | 61 МБ |
| экстрактор F0 `pm` (praat) | высота тона для RVC | **CPU** | — | — |
| energy VAD | нарезка эфира на фразы | **CPU** | — | — |
| ~~RMVPE~~ | нейросетевой F0 — **не используется** | — | было бы +335 МБ | 173 МБ лежат про запас |

В простое на карте около 4805 МиБ, на пике — 5029 из 5120.

Четыре решения, без которых стек в 5 ГБ не помещается:

- **turbo вместо `large-v3`** — 1124 против 2027 МиБ, на записи с рации качество то же;
- **`int8` в Whisper** — на Pascal FP16 идёт в 1/64 скорости, а CTranslate2 требует
  для него cc ≥ 7.0;
- **RVC в fp32** — по той же причине, но флагом вручную: авто-детект не узнаёт
  «Quadro P2000» по имени;
- **`pm` вместо `rmvpe`** — на вход RVC идёт чистый синтез Piper, а не шумный эфир,
  так что нейросетевой экстрактор не нужен: 335 МБ свободны, а сам расчёт уходит на CPU.

Piper остаётся на процессоре сознательно: конвейер строго последовательный, и он
работает ровно тогда, когда GPU уже отдал ответ LLM. На i7-2600 без AVX2 это
0.41–0.43 с на фразу — не узкое место. Перенос на карту дал бы выигрыш только на
бумаге, зато столкнул бы onnxruntime и CTranslate2 на общем cuDNN.

### Замеры по компонентам
...
| `llama-server` | 2678 МиБ |
| RVC (в простое 704, под нагрузкой до 1206) | ~1200 МиБ |
| Whisper turbo в агенте | 1124 МиБ |
| **на пике** | **5029 из 5120** |

Строка RVC здесь раньше гласила «~740 МиБ»: столько он занимает в покое и на коротких
фразах, но под нагрузкой берёт заметно больше — и именно это упускалось. Запас на
пике — около 90 МиБ, см. ниже.

### Пик RVC растёт с длиной фразы

Здесь раньше стояло обратное: «пик RVC не растёт с длиной, сервис режет вход на
куски». **Это неверно, и стоило двух OOM в эфире** (11.08.2026, 02:55 и 13:39).
Оба раза сценарий один: длинный ответ → RVC падает на нём с `torch.OutOfMemoryError`
→ агент передаёт голосом Piper, но на следующей же фразе падает сам, теперь уже на
Whisper → `Restart=always` поднимает его по кругу, потому что память держит сосед.
Лечилось перезагрузкой машины.

Нарезка в `pipeline.py` включается только для входа длиннее `x_max`, а точки реза
ставятся каждые `x_center` секунд — и оба числа `configs/config.py` выбирает **до**
того, как что-либо может переключить точность. `Quadro P2000` не подходит ни под один
его шаблон имени, поэтому `is_half` остаётся `True`, и берётся раскладка «для 6 ГБ»:

| | fp16 («6 ГБ»), что получалось | fp32 («5 ГБ»), что нужно |
|---|---|---|
| `x_pad` (добавляется дважды на кусок) | 3 с | 1 с |
| `x_center` (шаг точек реза) | 60 с | 38 с |
| `x_max` (порог нарезки) | 65 с | 41 с |

То есть при `--fp32` фраза короче 65 с шла в декодер **одним куском, да ещё и с 6 с
паддинга**: ответ на 14.76 с доезжал до сети как 20.76 с. Замер на dev (RTX 2070,
та же фраза, `--fp32`):

| нарезка | фраза 5.5 с | фраза 15.1 с |
|---|---|---|
| как было: `x_pad=3`, `x_max=65` | 1118 МиБ | **1452 МиБ** |
| пересчёт под fp32: `x_pad=1`, `x_max=41` | 974 МиБ | 1332 МиБ |
| **плюс рез по 6 с** (`--max-segment 6`) | 974 МиБ | **1058 МиБ** |

Рез снимает почти 400 МиБ и развязывает пик с длиной фразы — ценой 0.66 → 0.85 с на
конвертацию. Обычные ответы (короче `x_max`) идут одним куском, как и раньше, так что
на слух это касается только длинных. Сервис теперь пересчитывает раскладку сам, когда
ему передан `--fp32`, а `--max-segment` в юните доужимает её под 5 ГБ; проверять —
в `/health`, поле `chunking`.

**Замер на самой дачной машине**, все три службы подняты, `--max-segment 4`:

| | пик RVC |
|---|---|
| в простое | 704 МиБ |
| фраза 5.5 с | 928 МиБ |
| фраза 15.1 с | 1206 МиБ |

Было бы 1290+ — ровно столько RVC не смог выделить в ночь аварии. В юните стоит `4`,
а не `6`: по памяти на длинной фразе они равны (1206 против 1204), но `4` режет ещё и
промежуточные 7–9 с, которые при `6` шли одним куском.

**Ниже 4 без нужды не опускать.** `--max-segment 3` в лоб не работал: `x_query` —
это полуокно, в котором ищется точка реза, и upstream'овские 6 с не помещаются перед
первой точкой. Срез уходит за начало массива, возвращается пустым, и `.min()` падает
с `zero-size array to reduction operation minimum`. Наружу это выглядело как голый
500 на каждой длинной фразе — то есть агент молча передавал голосом Piper. Сервис
теперь ужимает `x_query` вместе с сегментом, но запас там небольшой.

Итог по всей карте на пике: **5029 из 5120 МиБ**, запас около 90. Он держится за счёт
того, что пик RVC перестал зависеть от длины фразы; если запас понадобится увеличить,
самый дешёвый ход — `-c 1536` вместо `-c 2048` у `llama-server` (в истории диалога
всего 8 коротких реплик, места хватит).

Со стороны агента ту же аварию страхуют ещё два рубежа (раздел «Длина ответа»
в README): `max_chars` обрезает текст по границе предложения, а где модель не
поставила ни одной точки — по слову, и `tts.max_seconds` — уже готовое аудио, до
отправки в RVC.
...
- [~] Боевой PTT: линия и логика проверены (диод на адаптере, 12.08.2026),
      осталось убедиться, что эфир слышен на второй рации
