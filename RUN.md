...
стек помещается на видеокарте рядом с десктопом.
...
## 3а. Врозь: мозг здесь, клиент у рации

Если рация подключена к другой машине в той же сети, вместо агента поднимите сервер —
флаги те же, что были у агента, минус всё про звук и PTT:
...
Порог, устройства и PTT — флаги клиента: они про его звуковую карту. Поставить
клиента на другую машину с нуля — [CLIENT.md](CLIENT.md). В журнале
клиента те же `[STT]`/`[LLM]` строки, что и у агента, плюс `[NET] сервер: 0.9 с (из
них сеть 0.02 с)` — по ней видно, что добавляет сеть. Провод до установки моделей
проверяется `serve --responder parrot`: клиент услышит своё эхо. Замер с сетью —
`main.py bench --in-file запись.mp3 --server http://<этот-хост>:8082`.
...
Сумма по этой таблице с профилем `vram5` — около 4.5 ГБ, плюс 2.5 ГБ десктопа: влезает
в восемь. **Но складывать значения в покое нельзя** — на этом уже обжигались. Whisper на
транскрипции и RVC на длинной фразе берут заметно больше, чем держат между фразами, и
по пикам дачная машина упиралась в 5029 из 5120 МиБ там, где сумма покоя обещала
вольготные 4.8. Замеры по пикам с обеих прод-машин — в DEPLOY.md, раздел 8.
...
| LLM | Qwen3-4B | Qwen3-4B | Qwen3-8B, `-c 2048` + KV `q8_0` |
